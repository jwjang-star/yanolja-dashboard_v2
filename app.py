...
# 💡 [전처리] 텍스트 공백 제거 및 숫자 변환
    ...
    # ══════════════════════════════════════════════════════════════════
    # 📖 메인 화면 토글형 매뉴얼 (폰트 크기 및 FAQ 가독성 최적화 버전)
    # ══════════════════════════════════════════════════════════════════
    import textwrap
    
    with st.expander("💡 실무자를 위한 대시보드 200% 활용 및 분석 프로세스 가이드 (클릭하여 펼치기)", expanded=False):
        # 💡 textwrap.dedent와 일반 문자열 기법으로 들여쓰기 깨짐을 원천 차단합니다.
        ...
        **Q. "점검 필요" 건수가 0인데도 가격이 이상한 것 같아요.** <br>A. 대시보드는 '중앙값 대비 30% 미만 또는 2배 초과'를 이상 단가로 감지합니다. <br>이 범위에 포함되지 않더라도 경쟁사 대비 높거나 낮을 수 있습니다. <br>TAB 3 상권별 비교를 통해 추가 확인하세요.

        ---

        **Q. 마감/미판매 비율이 갑자기 높아졌어요. 왜 그런가요?** <br>A. 주요 원인 세 가지를 순서대로 확인하세요:  <br>
        1. 채널매니저 연동 오류로 인한 자동 마감  <br>
        2. 현장에서 수동으로 마감 처리한 경우  <br>
        3. 특정 날짜 블로킹 설정 실수  <br>
        따라서 해당 지점 PMS에서 마감 처리 로그를 가장 먼저 확인하세요.<br>

        ---

        **Q. 대시보드 데이터가 실시간인가요?** <br> A. 데이터 추출 일시는 화면 상단 "추출 일시" 항목에서 확인 가능합니다. <br>일반적으로 당일 오전 기준 데이터가 반영됩니다. <br>실시간 모니터링이 필요한 경우에는 OTA 플랫폼 관리자 페이지를 직접 확인하세요.
        """)
