...
# 💡 [전처리] 텍스트 공백 제거 및 숫자 변환
    ...
    # 💡 순정 UI에서도 빨간색/초록색 경고 느낌을 살릴 수 있도록 delta 속성을 활용했습니다.
    if issue_cnt > 0:
    ...
    with st.expander("💡 실무자를 위한 대시보드 200% 활용 및 분석 프로세스 가이드 (클릭하여 펼치기)", expanded=False):
        ...
        ## 2. 일일 모니터링 루틴 (Daily Routine)

        ### ⏰ 출근 직후 (09:00~09:30) — "오늘의 긴급 점검"
        > **목표:** 비즈니스에 즉각적인 손실을 주는 이슈를 30분 안에 찾아 처리한다.

        ```text
        STEP 1. 대시보드 접속 → 상단 카드 4개 스캔 (1분)
                  ↓
        STEP 2. "점검 필요" 카드 숫자 확인
                  ├── 0건이면 → STEP 4로 이동
                  └── 1건 이상이면 → STEP 3 진행
                  ↓
        STEP 3. [TAB 1] 지점별 가격 현황 → "핵심 점검 사항" 표 확인
                  ├── 고단가 지점 목록 추출
                  ├── 저단가 지점 목록 추출
                  └── 해당 지점 현장 담당자에게 즉시 연락 (섹션 4 템플릿 활용)
                  ↓
        STEP 4. [TAB 1] 전체 가격 분포도 육안 확인 (5분)
                  └── 산점도에서 기준선(중앙값)과 동떨어진 점 없는지 시각적으로 재확인
                  ↓
        STEP 5. 마감/미판매 비율 확인
                  └── 전일 대비 마감 비율이 급증한 지점 있으면 → 재고 관리팀에 공유
        ```

        ### 📊 오전 업무 시간 (10:00~11:00) — "시장 포지셔닝 점검"
        > **목표:** 경쟁사 대비 우리 브랜드의 위치를 파악하고, 단가 조정 여부를 판단한다.

        ```text
        STEP 1. [TAB 2] 전 지점 다각도 랭킹 진입
                  ↓
        STEP 2. "대실" / "숙박" 탭 전환하며 확인
                  ├── 최저가(진입단가) 랭킹: 상위권인 지점은 가격 경쟁력 OK
                  ├── 중앙값(주력상품) 랭킹: 우리 브랜드의 실질적 수익 포지션
                  └── 최고가(프리미엄) 랭킹: 프리미엄 상품 라인업 경쟁력
                  ↓
        STEP 3. 중앙값 평균 기준선 대비 "하위 20%" 지점 식별
                  └── → 단가 인상 검토 대상으로 기록
                  ↓
        STEP 4. [TAB 3] 상권별 상세 비교
                  └── 경쟁 열위(우리 가격 > 경쟁사 가격) 지점 확인 → 전략 검토
        ```

        ### 📝 일일 업무 마감 전 (17:00~17:30) — "당일 조치 사항 기록"
        * 오늘 처리한 이상 단가 건수 및 조치 내용 기록
        * 현장 담당자 피드백 수신 여부 확인
        * 내일 팔로업이 필요한 지점 목록 업데이트

        ---

        ## 3. TAB별 데이터 해석 및 액션 플랜
        ...
        **원인 분류 및 해결 방법:**
        ...
        ## 4. 현장 커뮤니케이션 가이드
        > **원칙:** 현장 담당자에게 연락할 때는 **데이터를 근거**로, **구체적인 수치**와 함께, **명확한 요청 사항**을 전달합니다. 막연한 "가격 좀 확인해 주세요"는 지양합니다.

        ### 📞 템플릿 1: 이상 고단가 즉시 수정 요청
        ```text
        [긴급 단가 확인 요청]

        ○○○ 담당자님,
        OTA 가격 모니터링 대시보드에서 이상 단가가 감지되어 즉시 확인을 요청드립니다.

        ■ 발생 지점: [지점명]
        ■ 해당 상품: [객실 타입명]
        ■ 현재 등록 가격: [금액]원
        ■ 당일 지점 중앙값: [금액]원 (현재 가격은 중앙값 대비 약 [배수]배 수준)
        ■ 발견 시각: [HH:MM]

        [조치 요청 사항]
        1. PMS 또는 채널매니저에서 해당 객실 요금 원인 확인
        2. 오입력 확인 시 즉시 수정 후 OTA 동기화
        3. 의도적 설정이라면 사유를 회신 주시기 바랍니다.

        수정 완료 후 본 메시지에 회신 부탁드립니다.
        처리 목표: 접수 후 2시간 이내
        ...
        ### 📞 템플릿 2: 경쟁사 대비 단가 인상 권고
        ```text
        [단가 조정 검토 요청]

        ○○○ 담당자님,
        이번 주 상권별 가격 비교 분석 결과를 공유드립니다.

        [분석 내용]
        현재 [지점명]의 숙박 중앙값은 [금액]원으로,
        동일 상권 내 경쟁사 평균 [금액]원 대비 약 [X]% 낮은 수준입니다.
        고객 유입 데이터상 해당 상권의 예약 전환율은 양호하므로,
        현 가격 수준에서의 점진적 인상이 수익성 개선에 도움이 될 것으로 판단됩니다.

        [권고 조치]
        - 숙박 주력 상품(중간 등급 객실) 단가: 현행 대비 5~10% 인상 검토
        - 인상 시작일: [날짜] 이후 적용 권장

        담당자님의 현장 상황 및 의견을 회신 주시면
        최종 가격 전략 수립 시 반영하겠습니다.
        ...
        **Q. "점검 필요" 건수가 0인데도 가격이 이상한 것 같아요.**
        A. 대시보드는 '중앙값 대비 30% 미만 또는 2배 초과'를 이상 단가로 감지합니다. 이 범위에 포함되지 않더라도 경쟁사 대비 높거나 낮을 수 있습니다. TAB 3 상권별 비교를 통해 추가 확인하세요.

        **Q. 마감/미판매 비율이 갑자기 높아졌어요. 왜 그런가요?**
        A. 주요 원인 세 가지를 순서대로 확인하세요: ① 채널매니저 연동 오류로 인한 자동 마감 ② 현장에서 수동으로 마감 처리한 경우 ③ 특정 날짜 블로킹 설정 실수. 해당 지점 PMS에서 마감 처리 로그를 확인하세요.

        **Q. 대시보드 데이터가 실시간인가요?**
        A. 데이터 추출 일시는 화면 상단 "추출 일시" 항목에서 확인 가능합니다. 실시간 모니터링이 필요한 경우에는 OTA 플랫폼 관리자 페이지를 직접 확인하세요.
        """)
